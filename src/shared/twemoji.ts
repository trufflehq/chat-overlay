...
export function parseEmojis(text: string) {
  const results: (string | ChatObject)[] = [];
  let match;
  let lastIndex = 0;

  while ((match = emojiRegExp.exec(text)) !== null) {
    // If there is text before the emoji, add it as a regular string.
    if (match.index > lastIndex) {
      results.push(text.slice(lastIndex, match.index));
    }

    // Use the grabTheRightIcon function to get the code point.
    const icon = grabTheRightIcon(match[0]);

    // Create a ChatObject for the emoji.
    const chatObject: ChatObject = {
      type: 'emoji', // You might need more logic to differentiate between 'emoji', 'emote', etc.
      url: `https://twemoji.maxcdn.com/svg/${icon}.svg`, // This URL should be the location where your emoji images are hosted.
      name: icon,
    };

    // Add the ChatObject to the results array.
  ...
  // If there is text after the last emoji, add it as a regular string.
